...
def save_to_file(data, ip_address):
    filename = f"{ip_address}-services.org"
    with open(filename, 'w') as f:
        for entry in data:
            f.write(f"*** Services {entry['portid']}/{entry['protocol']} {entry['service_name']} ({entry['product']} {entry['version']})\n")
...
def main():
    ...
    save_to_file(extracted_data,  ip_address)
